...
app.get("/products", (req, res) => {
    res.send({msg: "Product list", items: products})
});
...
app.get("/products/nameProduct", (req, res) => {
    const found = products.filter(product => product.name == req.query.name);
    
    if(found) {
        res.send({Result:"Product selected:", found});
    } else {
      res
        .status(404)
        .send({ msg: `Product with name ${req.query.name} not found` });
    }
  });

  //En este ejercicio en la barra de nav o en postaman se pone asi, ejemplo:
  //localhost:8080/products/nameProduct?name=Skin%20Valorant
  //%20 simula un espacio

 //FIXME: filtro por id


//   app.get("/products/:id", (req, res) => {
//     const found = products.filter(product => product.id == req.params.id);
...
//       res
//         .status(404)
//         .send({ msg: `Product with name ${req.params.id} not found` });
//     }
//   });
